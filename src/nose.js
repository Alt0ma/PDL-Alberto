...
let uno int = 1;	// la inicializacion es de implementacion opcional
let UNO int= uno;
...
function FactorialWhile int ()
{
	...
	let i	int;			// variables locales: i inicializada a 0 por omision
...
{
	let i int;
	let factorial int = UNO;	/* declaracion de variables locales */
	for (i = 1; i <= n; i++)
	{
		factorial *= i;
	}
...
function imprime (string s, string msg, int f)	/* funcion que recibe 3 argumentos */
{
	print s;print msg ;print (f);
	print salto();	// imprime un salto de linea */
...
print
 "\nIntroduce un 'numero'.";
input
 num;	/* se lee un numero del teclado y se guarda en la variable global num */
...
function demo ()	/* definicion de la funcion demo, sin argumentos y que no devuelve nada */
{
	...
	if (!((v1 == v2) && (v1 != v3)))	/* NOT ((v1 igual a v2) AND (v1 distinto de v3))  */
	{
		...
		{
			let  v0 int= v2; // se declara v0 aqui, por lo que se puede utilizar hasta el final de la funcion
		}
		else
		{
			v0= 1 / v3;
